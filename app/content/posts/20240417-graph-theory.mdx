...
Graph theory revolves around graphes... evidently. Graphs are objects composed
...
are connected is reduced to $O(1)$, whereas the adjacency list suffers a $O(n)$ time
complexity.
...
A bipartite graph is composed of two graphes with null edge sets, so $G=(E_1 + E_2, V)$.
