...
```math
f(x) = a^x = e^{\ln(a)x} \\
\frac{df(x)}{dx} = \ln(a) \cdot e^{\ln(a)x} = \ln(a) \cdot f(x)
```
...
```math
\frac{d(\ln f(x))}{dx} = \frac{d(\ln f(x))}{df} \cdot \frac{df(x)}{dx} = \frac{1}{f(x)} \cdot \frac{df(x)}{dx}
```
